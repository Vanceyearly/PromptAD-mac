...
def fit(model,
    ...
    features1 = []
    features2 = []
    for (data, mask, label, name, img_type, aug_data, perlin_mask) in tqdm(train_data, desc="Building feature gallery"):
    ...
    for cur_epoch in epoch_progress:
        ...
        for (data, mask, label, name, img_type, aug_data, perlin_mask) in data_progress:
            data = [model.transform(Image.fromarray(cv2.cvtColor(f.numpy(), cv2.COLOR_BGR2RGB))) for f in data]
            data = torch.stack(data, dim=0).to(device)
            data = data.to(device)

            ab_data = [model.transform(Image.fromarray(cv2.cvtColor(f.numpy(), cv2.COLOR_BGR2RGB))) for f in aug_data]
            ab_data = torch.stack(ab_data, dim=0).to(device)
            ...
            _, ab_feature_map, _, _ = model.encode_image(ab_data)
        ...
        for (data, mask, label, name, img_type, aug_data, perlin_mask) in dataloader:
