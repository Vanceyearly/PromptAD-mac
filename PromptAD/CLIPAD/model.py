...
class CLIP(nn.Module):
    ...
    def encode_image(self, image, normalize: bool = False):
        ...
        # features: pooled, tokens, feature1, feature2
        return [F.normalize(feature, dim=-1) if normalize and feature is not None else feature for feature in features]
    ...
    def encode_fused_features(self, image, text):
        # 获取视觉和文本的中间特征
        pooled_v, tokens_v, feature1, feature2 = self.visual(image)
        pooled_t, tokens_t, text_feature1, text_feature2 = self.textual(text)
        # 视觉特征投影到文本特征维度
        if feature1 is not None and text_feature1 is not None:
            vis1_proj = self.vis1_to_text(feature1)
            fused1 = vis1_proj + text_feature1
        else:
            fused1 = None
        if feature2 is not None and text_feature2 is not None:
            vis2_proj = self.vis2_to_text(feature2)
            fused2 = vis2_proj + text_feature2
        else:
            fused2 = None
