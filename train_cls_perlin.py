    ...
    for cur_epoch in epoch_progress:
        ...
        for (data, mask, label, name, img_type, perlin_data, cutpaste_data) in data_progress:
            ...
            if False:
                ...
                for i, (d, aug, perlin, n) in enumerate(zip(data, perlin_data, cutpaste_data, name)):
                    # 检查图像数据的形状
                    print(f"Original image shape: {d.shape}")
                    print(f"Augmented image shape: {aug.shape}")
                    print(f"Perlin mask shape: {perlin.shape}")

                    # 保存原始图像
                    original_image_path = os.path.join(save_dir, f'{n}_original.png')
                    cv2.imwrite(original_image_path, (d.cpu().numpy() * 255).astype(np.uint8))

                    # 保存增强图像
                    perlin_image_path = os.path.join(save_dir, f'{n}_perlin_image.png')
                    cv2.imwrite(perlin_image_path, (aug.cpu().numpy() * 255).astype(np.uint8))

                    # 保存Perlin掩码
                    cutpaste_image_path = os.path.join(save_dir, f'{n}_cutpaste_image.png')
                    cv2.imwrite(cutpaste_data_path, (perlin.cpu().numpy() * 255).astype(np.uint8))
            ...
            loss_v2t_ab = criterion(logits_v2t_ab, target_v2t_ab)
