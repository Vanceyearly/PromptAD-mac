...
def fit(model,
    ...
    for cur_epoch in epoch_progress:
        ...
        for (data, mask, label, name, img_type, perlin_data, cutpaste_data) in data_progress:
            data = [model.transform(Image.fromarray(cv2.cvtColor(f.numpy(), cv2.COLOR_BGR2RGB))) for f in data]
            data = torch.stack(data, dim=0).to(device)
            data = data.to(device)
            ...
            cp_data = [model.transform(Image.fromarray(cv2.cvtColor(f.numpy(), cv2.COLOR_BGR2RGB))) for f in cutpaste_data]
            cp_data = torch.stack(cp_data, dim=0).to(device)
            cp_data = cp_data.to(device)
            ...
            _, pl_feature_map, _, _ = model.encode_image(pl_data)
            _, cp_feature_map, _, _ = model.encode_image(cp_data)

            alpha = args.a
            ab_feature_map = (1 - alpha) * pl_feature_map + alpha * cp_feature_map
            ...
            l_pos_ab = torch.einsum('nic,cj->nij', ab_feature_map, abnormal_text_features_ahchor.transpose(0, 1))
            l_neg_v2t_ab = torch.einsum('nic,cj->nij', ab_feature_map, normal_text_features.transpose(0, 1))
            ...
            target_v2t_ab = torch.zeros([logits_v2t_ab.shape[0], logits_v2t_ab.shape[1]], dtype=torch.long).to(device)
            loss_v2t_ab = criterion(logits_v2t_ab.transpose(1, 2), target_v2t_ab)

            trip_loss = criterion_tip(feature_map, normal_text_features_ahchor, abnormal_text_features_ahchor)
            loss = loss_v2t + trip_loss + loss_match_abnormal * args.lambda1 + loss_v2t_ab

            print(f"loss: {loss.item():.4f}, loss_v2t: {loss_v2t.item():.4f}, loss_v2t_ab: {loss_v2t_ab.item():.4f}, trip_loss: {trip_loss.item():.4f}, loss_match_abnormal: {loss_match_abnormal.item():.4f}")
